...
public class FormulaUnitTest
{
    ...
    public void testBasics() throws Exception
    {
        ...
        assertEquals(0.0, f.eval().getDouble(0), epsilon);

        // floating point
        f = new Formula("-3.14");
        assertEquals(-3.14, f.eval().getDouble(0), epsilon);

        // exponential
        f = new Formula("-2.123e4");
        assertEquals(-2.123e4, f.eval().getDouble(0), epsilon);

        // exponential
        f = new Formula("-2.123e-14");
        assertEquals(-2.123e-14, f.eval().getDouble(0), epsilon);

        f = new Formula("-3.14 + 2");
        assertEquals("(-3.14 + 2.0)", f.toString());
        assertEquals(-1.14, f.eval().getDouble(0), epsilon);

        f = new Formula("-3.14 - 2");
        assertEquals(-5.14, f.eval().getDouble(0), epsilon);

        f = new Formula("-3.14 + 2 - 1.10");
        assertEquals(-2.24, f.eval().getDouble(0), epsilon);

        f = new Formula("-12/-3");
        assertEquals(4.0, f.eval().getDouble(0), epsilon);

        // Order, quotes
        f = new Formula("1 + 2 * 3 - 4");
        assertEquals(3.0, f.eval().getDouble(0), epsilon);
    ...
    public void testBool() throws Exception
    {
        ...
        assertEquals(1.0, f.eval().getDouble(0), epsilon);
    ...
    public void testFunctions() throws Exception
    {
        ...
        assertEquals(2.0, f.eval().getDouble(0), epsilon);
        ...
        assertEquals(1.414, f.eval().getDouble(0), epsilon);

        f = new Formula("min(5, 4, 3, 2, 1)");
        assertEquals(1, f.eval().getDouble(0), epsilon);

        f = new Formula("max(5, 4, 3, 2, 1)");
        assertEquals(5, f.eval().getDouble(0), epsilon);

        f = new Formula("sin(" + Math.toRadians(30) + ")");
        assertEquals(0.5, f.eval().getDouble(0), epsilon);

        f = new Formula("sin(toRadians(30))");
        assertEquals(0.5, f.eval().getDouble(0), epsilon);

        f = new Formula("cos(30)");
        assertEquals(0.1543, f.eval().getDouble(0), epsilon);

        f = new Formula("atan2(10.0, 0.0)");
        assertEquals(90.0, Math.toDegrees(f.eval().getDouble(0)), epsilon);

        f = new Formula("rnd(10.0)");
        for (int i=0; i<50; ++i)
        {
            double rnd = f.eval().getDouble(0);
            assertTrue(rnd >= 0.0);
            assertTrue(rnd < 10.0);
            double rnd2 = f.eval().getDouble(0);
    ...
    public void testVariables() throws Exception
    {
        ...
        v[0].setValue(20.0);
        v[1].setValue(30.0);
        assertEquals(300.0, f.eval().getDouble(0), epsilon);
        ...
        assertEquals(2.0*Math.PI, f.eval().getDouble(0), epsilon);

        v[0] = new VariableNode("PI", 10.0);
        f = new Formula("PI", v);
        assertEquals(10.0, f.eval().getDouble(0), epsilon);
    ...
    public void testErrors() throws Exception
    {
        ...
        f = new Formula("1/0");
        assertTrue(Double.isInfinite(f.eval().getDouble(0)));
